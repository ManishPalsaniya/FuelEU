...
import { ComplianceRepository, RouteRepository, BankingRepository } from '../ports/Repositories';
...
export class ComplianceUseCase {
    ...
    async calculateComplianceBalance(shipId: string, year: number): Promise<ComplianceBalance> {
        ...
        // Calculate CB
        const cbVal = Formulas.calculateComplianceBalance(route.ghgIntensity, route.fuelConsumption);
        ...
        if (!compliance) {
            compliance = new ComplianceBalance(
                '', // ID generated by DB/Repo
                shipId,
                year,
                cbVal,
                cbVal, // Before banking
